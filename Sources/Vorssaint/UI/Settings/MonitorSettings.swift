...
private struct ConnectedDevicesMenuBarOrderOptions: View {
    ...
    var body: some View {
        if menuBarConnectedDevices {
            MetricRowOption(symbol: MenuBarMetric.connectedDevices.symbolName,
                            label: l10n.s.usbExcludeChargers,
                            isOn: $excludeChargers)
            MetricRowOption(symbol: MenuBarMetric.connectedDevices.symbolName,
                            label: l10n.s.usbExcludeHubs,
                            isOn: $excludeHubs)
            MetricRowOption(symbol: MenuBarMetric.connectedDevices.symbolName,
                            label: l10n.s.usbExcludeEthernet,
                            isOn: $excludeEthernet)
            MetricRowOption(symbol: MenuBarMetric.connectedDevices.symbolName,
                            label: l10n.s.usbExcludeStorage,
                            isOn: $excludeStorage)
        }
    }
}
