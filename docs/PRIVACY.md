...
5. **Video Downloader, only after you enter a valid link.** Inspection and download contact the pasted site from your Mac through the locally installed `yt-dlp`, using the explicitly discovered Deno JavaScript runtime. FFmpeg performs media conversion and `ffprobe` verifies the finished file before publication. Inspection starts only after the text is a valid HTTP or HTTPS URL. When inspection supplies a remote preview-image URL, the app loads that image directly from its host to show the thumbnail. The media URL is not saved, logged or placed in command-line arguments; it is sent to `yt-dlp` over standard input. The app runs yt-dlp with an explicit deterministic policy: it ignores local config files and custom plugins, confines caching to an app-owned directory (used only for yt-dlp's EJS challenge-solver scripts, which are fetched from GitHub on demand when a site demands a JavaScript challenge and cached there so the fetch happens once), and does not inherit `.netrc`, cookie files or browser sessions. There is no implicit local authentication; the only exception is an explicit, opt-in setting that lets yt-dlp read login cookies from one chosen browser so age-gated or member-only content can be inspected and downloaded — those cookies are read only during inspection and download, never stored by the app, and the setting is off by default. The app also disables configured `--exec` shell hooks. The feature inspects one media item at a time and rejects playlists, live or upcoming items, private or restricted media, and DRM-only items; when inspection metadata is unavailable, the app keeps the link downloadable and lets yt-dlp decide. Installing missing `yt-dlp`, `ffmpeg` (which provides `ffprobe`) or `deno` tools may contact Homebrew, GitHub and package hosts; when Homebrew itself is absent, the official installer opens in Terminal and may ask for confirmation or a password.
